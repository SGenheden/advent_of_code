...
def solve(input_rows):
    ...
    complete_boards = [False] * len(board_numbers_list)
    for number in numbers:
        for board_idx, (board_numbers, board_taken) in enumerate(
            zip(board_numbers_list, board_taken_list)
        ):
            if complete_boards[board_idx]:
                continue
            if update_board(board_numbers, board_taken, number):
                complete_boards[board_idx] = True
                if all(complete_boards):
                    unmarked_sum = sum(
